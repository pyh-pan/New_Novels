...
- 客户端内部导航和 API 请求必须经过 `lib/app/runtime-paths.ts` 或 `components/AppLink.tsx`。应用入口路径优先来自服务端写入的 `new-novels-entry-path`，其次来自平台 `<base href>`，最后从当前页面路径反推；所有页面只在入口路径后追加 `/cases/...`、`/studio...`、`/api/...` 等业务后缀。不要在组件中裸跳转到 `/cases/...` 或 `/api/...`，否则部署在子路径时会离开子应用并触发平台 404。
